...
struct user
{
    ...
    void return_copy(int bookId)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            if (borrowed_books_ids[i] == bookId)
            {
    ...
    void print()
    {
        // sort borrowed_books_ids
        sort(borrowed_books_ids, borrowed_books_ids + len);
        cout << "id = " << id << ", name = " << name << ", borrowed books ids = ";
        for (int i = 0; i < len; i++)
        {
